...
const App = (() => {
  ...
  const treeSvg            = document.getElementById('tree-container');
  ...
  // Show an error in the loading overlay for 7 seconds then auto-dismiss
  ...
  async function evolve() {
    ...
    try {
      const res = await api('/api/evolve', { session_id: sessionId, mutation_strength: parseFloat(appMutationSlider.value) });
